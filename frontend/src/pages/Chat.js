...
// ✅ API base — empty string means relative URLs, nginx proxies /api/ to backend
...
export default function Chat() {
  ...
  // ✅ All API calls now use relative paths — nginx proxies /api/ → backend:5005
  const fetchHistory = async () => {
    try {
  ...
  const fetchModels = async () => {
    try {
      const res = await axios.get(`${API_BASE}/api/chat/models/list`);
      if (res.data.models?.length > 0) {
        setAvailableModels(res.data.models.map(m => m.name));
        setModel(res.data.models[0].name);
      }
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      stopThinkingAnimation();
      setIsTyping(false);
      setMessages(prev => [...prev, { role: "assistant", content: "❌ Error connecting to Ollama. Make sure Ollama is running.", id: Date.now() }]);
    }
  };

  // ✅ All axios calls use relative paths
  const loadChat = async (chatId) => {
    try {
